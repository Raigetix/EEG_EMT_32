...
void DAC7678::write(uint8_t command, uint16_t data)
{
    ...
    wire->write(data >> 8);  // transmit data byte 1 (MSB)
    wire->write(data & 0xFF); // transmit data byte 0 (LSB)
 ...
 */
void DAC7678::reset(SpeedMode mode)
{
 ...
 */
uint8_t DAC7678::getChannelCode(int chan1, int chan2, int chan3, int chan4, 
                                int chan5, int chan6 , int chan7, int chan8)
{
